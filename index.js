// Future project: create a comments board on each page, I might covert to react as this would be a great component
// <commentBox dbName="food" name="food" />   use one component instance on each page
$(function() {
  var currentBoard = window.location.pathname.slice(3, -1); // this is key!!! get the name of the board based on the path
  var url = "/api/threads/" + currentBoard;
  $("#boardTitle").text("Welcome to Board" + window.location.pathname); // this board is by design the pathname from API call
  $.ajax({
    type: "GET",
    url: url,
  ...
  $("#boardDisplay").on("submit", "#reportThread", function(e) {
    ...
    $.ajax({
      ...
      data: $(this).serialize(),
      success: function(data) {
        console.log("reporting thread ", data);
        alert(data);
      }
    });
    e.preventDefault();
